...
%       what case is short range and low base station height
        ...
        elseif ( freq>150 && freq<= 1500)
           ...
           L(d2)=69.6 + 26.2*log10(freq)- 13.82*log10(max(30,Hb))+(44.9 - 6.55*log10(max(30,Hb)))*(log10(dist(d2)).^alfa(d2)) -a-b;

        elseif ( freq>1500 && freq<= 2000)

           L(d2)=46.3 + 33.9*log10(freq)- 13.82*log10(max(30,Hb))+(44.9 - 6.55*log10(max(30,Hb)))*(log10(dist(d2)).^alfa(d2)) -a-b;

        elseif ( freq>2000 && freq<= 3000)

            L(d2)=46.3 + 33.9*log10(2000) + 10*log10(freq/2000) - 13.82*log10(max(30,Hb))+(44.9 - 6.55*log10(max(30,Hb)))*(log10(dist(d2)).^alfa(d2)) -a-b;
